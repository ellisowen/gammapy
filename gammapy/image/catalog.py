...
from gammapy.datasets.load import fetch_fermi_extended_sources
...
def catalog_image(psf='Fermi', resolution=0.1, center=[0, 0], lat_range=[0, 180], lon_range=[0, 360],
    ...
    if psf == 'None':
        # If there is no PSF defined, sources will be modeled as Gaussian
    ...
    if source_type == 'ExtendedSource' or 'All':
        hdu_list = fetch_fermi_extended_sources(catalog)
        for source in hdu_list:
            source_wcs = WCS(source.header)
            source_spec_cube = GammaSpectralCube(data = source.data, wcs=source_wcs, energy=energy)
        ...
        source_list = np.arange(len(source_table['Flux'])).astype(int)
    ...
    if psf == 'Gaussian':
        ...
        for i in np.arange(len(energy) - 1):
            ...
            convolved_cube[i] = convolve(new_image, kernel_image,
                                         mode='constant')
...
def catalog_table(catalog, ebands='No'):
    ...
    data = []
    for source in np.arange(len(fetch_fermi_catalog(catalog, 'LAT_Point_Source_Catalog'))):
        ...
        if catalog == '1FHL':
            ...
            if ebands == 'No':
                ...
                Flux_10_30 = fetch_fermi_catalog(catalog, 'LAT_Point_Source_Catalog')['Flux10_30GeV'][source]
                Flux_30_100 = fetch_fermi_catalog(catalog, 'LAT_Point_Source_Catalog')['Flux30_100GeV'][source]
                Flux_100_500 = fetch_fermi_catalog(catalog, 'LAT_Point_Source_Catalog')['Flux100_500GeV'][source]
            ...
            if ebands == 'No':
                ...
                Flux_30_100 = fetch_fermi_catalog(catalog, 'LAT_Point_Source_Catalog')['Flux30_100'][source]
                Flux_100_300  = fetch_fermi_catalog(catalog, 'LAT_Point_Source_Catalog')['Flux100_300'][source]
                Flux_300_1000  = fetch_fermi_catalog(catalog, 'LAT_Point_Source_Catalog')['Flux300_1000'][source]
                Flux_1000_3000  = fetch_fermi_catalog(catalog, 'LAT_Point_Source_Catalog')['Flux1000_3000'][source]
                Flux_3000_10000  = fetch_fermi_catalog(catalog, 'LAT_Point_Source_Catalog')['Flux3000_10000'][source]
                Flux_10000_100000  = fetch_fermi_catalog(catalog, 'LAT_Point_Source_Catalog')['Flux10000_100000'][source]
...
if __name__ == '__main__':
    catalog_image(psf='Fermi', resolution=0.1, center=[0, 0], lat_range=[0, 180], lon_range=[0, 360],
                  catalog='1FHL', source_type='All', total_flux='False', filename='1fhl_fermi_psf.fits', energy = Quantity([10, 500], 'GeV'))
