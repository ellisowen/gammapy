...
def _interp_flux(hdu, new_energy):
    cube = GammaSpectralCube.read_hdu(hdu)
    from gammapy.image import cube_to_image
    image_hdu = cube_to_image(hdu, 0)
...
def _interp_exposure(hdu, new_energy):
...
def _reproject_cube(hdu1, hdu2, smooth=False):
...
def compute_npred_cube(flux_hdu, exposure_hdu, desired_energy=None,
    ...
    if desired_energy != None:
        ...
        exposure = _interp_exposure(exposure_hdu, desired_energy)
    else:
        flux = flux_rep.data
        exposure = exposure_hdu.data

    flux *= exposure

    if convolve == 'Yes':
        energy_bins = flux_hdu.header['NAXIS3']
        energy_min = flux_hdu.header['CRPIX3']
        bin_size = flux_hdu.header['CDELT3']
        resolution = flux_hdu.header['CDELT2']
        energy_max = energy_min + energy_bins * bin_size
        ...
        if len(flux.shape) == 3:
            for index in indices:
                ...
                counts[index] = _correlate_fermi_psf(cube_layer, max_convolution_offset,
                                                     resolution, energy,
                                                     energy_band=[eband_min, eband_max])
        elif len(flux.shape) == 2:
            energy = 0.5 * (energy_min + energy_max)
...
class GammaSpectralCube(object):
    ...
    * Methods use the ``wcs`` order of ``(lon, lat, energy)``,
    ...
    def read_hdu(hdu_object):
        ...
        header = hdu_object.header
        wcs = WCS(header)
        energy_bins = np.arange(header['NAXIS3'])
        energy_min = (header['CRPIX3']) * np.ones_like(energy_bins)
        bin_size = header['CDELT3']
        energy = energy_min + bin_size * energy_bins
        energy = Quantity(energy, 'MeV')
