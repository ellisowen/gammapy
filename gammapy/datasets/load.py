...
def fetch_fermi_extended_sources(catalog):
    ...
    * 1FHL Catalog : First Fermi-LAT Catalog of Sources above 10 GeV
    ...
    hdu_list = []
    for member in np.arange(len(tar.getmembers())):
        mem_object = tar.getmembers()[member]
        mem_filename = mem_object.name
        if mem_filename.endswith(".fits") == True:
            hdu = fits.open(mem_filename)[0]
            hdu_list.append(hdu)
